#!/bin/bash
# File: /home/vince/IxomeAI/frontend/push-to-github-fixed.sh
# Purpose: Fix git config, add remote with org, amend commit, push to ixome-ai/ixome-frontend using bronn2120 PAT – Dec 02, 2025
...
if [ ! -f "$FRONTEND_DIR/.env" ]; then
  echo "Error: .env not found! Run setup-github-env-fixed.sh first."
  exit 1
fi
...
echo "Verification: git log -1 (check commit); open GitHub repo to confirm files uploaded."
...
echo "Monitoring: Check GitHub for push notification and access logs; rotate PAT if compromised."